...
from agentseek_langchain.shapes import HumanMessageContent, as_str_mapping, copy_str_mapping, model_dump_or_none
...
def _serialize_structured_value(value: object) -> str:
    if value is None:
        return ""
    if (dumped := model_dump_or_none(value)) is not None:
...
def parse_text_output(result: object) -> str:
    ...
    if mapping is not None:
        ...
        if "messages" in mapping:
            return _extract_text_from_messages(mapping["messages"])
...
def parse_messages_output(result: object, *, messages_key: str = "messages") -> str:
    ...
    if mapping is not None:
        ...
        if structured is not None:
            return _serialize_structured_value(structured)
        if messages_key in mapping:
            return _extract_text_from_messages(mapping[messages_key])
...
def _build_langchain_messages(
    ...
    *,
    ...
    fallback_messages.append(HumanMessage(content=_prompt_to_human_content(context.prompt)))
...
def _extract_text_from_messages(messages: object) -> str:
    ...
    for message in reversed(collected):
        ...
        mapping = as_str_mapping(message)
        if mapping is not None:
            content = mapping.get("content")
            if content is not None:
                return _render_message_content(content)
            data = as_str_mapping(mapping.get("data"))
            if data is not None and "content" in data:
                return _render_message_content(data["content"])
    if not collected:
        return ""
...
def _render_message_content(content: object) -> str:
    ...
    mapping = as_str_mapping(content)
    if mapping is not None:
        if isinstance(mapping.get("text"), str):
            return str(mapping["text"])
        return json.dumps(copy_str_mapping(mapping), ensure_ascii=True, sort_keys=True)
    return str(content)
...
def _prompt_to_human_content(prompt: str | list[dict[str, Any]]) -> HumanMessageContent:
