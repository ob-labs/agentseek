...
export default function App() {
  ...
  return (
    <main>
      ...
      <section className="chat" aria-label="Content conversation">
        {rows.length === 0 && (
          <p className="hint">
            Try: <em>"Write a blog post about how AI agents are transforming software development"</em>
          </p>
        )}
        {rows.map((row) =>
          row.kind === "prose" ? (
            ...
            <ToolCallCard key={row.key} card={row.card} apiUrl={apiUrl} />
          ),
        )}
        {stream.isLoading && (
          <div className="activity-card" aria-live="polite" aria-label="Content generation in progress">
