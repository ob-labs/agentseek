...
from .hybrid import weighted_fuse, weights_for_mode
...
def _format_trace(
    ...
    hits = weighted_fuse(vector_hits, sparse_hits, fulltext_hits, weights, top_k)
    return SearchTrace(
        query=query,
        mode=mode,
        weights=weights,
...
class HybridImageStore:
    ...
    def _search_routes(
        ...
        sparse_results = fulltext_results
        return _format_trace(
            query=query,
            mode=mode,
            vector_hits=self._query_hits(vector_results),
            sparse_hits=self._get_hits(sparse_results),
