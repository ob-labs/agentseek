...
This directory holds [cookiecutter](https://cookiecutter.readthedocs.io/) projects that `agentseek new` scaffolds for users. Templates are discovered by `_local_templates_root()` in `src/agentseek/cli/commands/new.py`; the CLI resolves `<type>/<name>` to `templates/<type>/<name>/`.
...
- `<type>` is one of `bub`, `deepagents`, `langchain` (the values in `KNOWN_TYPES` at `src/agentseek/cli/commands/new.py`).
...
- **Generated layout** — a tree of what `agentseek new <type>/<name>` produces.

The generated project's `README.md` (inside `{{cookiecutter.project_slug}}/`) must include:

- **Setup** — the exact commands a user runs after `agentseek new`.
...
2. Run the render check: `uv run pytest tests/cli_commands/test_templates_render.py -v`. Your template should appear as a new parametrize case and pass.
