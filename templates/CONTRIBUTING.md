...
This directory holds [cookiecutter](https://cookiecutter.readthedocs.io/) projects that `agentseek create` scaffolds for users. Templates are discovered by `_local_templates_root()` in `src/agentseek/cli/commands/create.py`; the CLI resolves `<type>/<name>` to `templates/<type>/<name>/`.
...
- `<type>` is one of `bub`, `deepagents`, `langchain` (the values in `KNOWN_TYPES` at `src/agentseek/cli/commands/create.py`).
