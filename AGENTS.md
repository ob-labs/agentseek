...
## Chat channels (`$dingtalk`, `$wechat`, `$feishu`, `$discord`, …)
...
- **Channel-specific send tools** (names vary by plugin: e.g. `dingtalk_send`, `wechat`, Discord/Feishu/Telegram helpers): use them only when you must send a message **from inside another tool** (e.g. progress during a long task), not for normal turn replies.
