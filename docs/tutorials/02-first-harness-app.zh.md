...
这会在 `my_bub_agent/` 里（而不是在 agentseek checkout 里）创建一个 `.venv/`，并安装 `agentseek`、`agentseek-ag-ui` 和其它列出的依赖。如果你是从本地源码 checkout 生成的项目，`pyproject.toml` 已经通过 `[tool.uv.sources]` 指向它了 —— 完整对照表见 [模板参考](../reference/templates.zh.md)。
...
填入 `AGENTSEEK_API_KEY`（如果你不用 OpenAI，再填 `AGENTSEEK_API_BASE`）。模型也可以换 —— `openrouter:free`、`openai:qwen-plus` 等等。变量名和 CLI 用的是同一套，因为模板依赖的是同一个 agentseek 发行版。完整参考在 [环境变量参考](../reference/environment.zh.md)。
...
模型路由决策位于 `agentseek-ag-ui`（一个 contrib 包）和 `agentseek` 发行版本身；一轮对话如何从 channel 经过 runtime 走到模型，见 [运行时数据模型](../explanation/runtime-data-model.zh.md)。
