...
（`new / dev / build / deploy / api / ctx / skills`），`agentseek` 是
...
| A2-LC —— LangChain / DeepAgents 开发者 | [agentseek 与 LangChain 的关系](../explanation/langchain-relationship.zh.md) | [模板参考](../reference/templates.zh.md)——入门选 `langchain/markdown-messages`，交付产品选 `langchain/default` |
...
2. **[构建你的第一个 harness 应用](02-first-harness-app.zh.md) —— 你的第一个 harness 应用。** 面向应用开发者的路径。先用 `agentseek new`（由 `agentseek-cli` 提供）生成项目，再在生成项目里执行 `uv sync`，把 harness 本体解析进去。看完这一页之后，你持续编辑的就是这个生成出来的项目，而不是 clone 下来的仓库。
