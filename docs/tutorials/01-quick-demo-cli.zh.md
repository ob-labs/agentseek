...
agentseek 读取 `AGENTSEEK_*` 变量，并以 `BUB_*` 别名转发给底层的 Bub runtime。映射定义在 `src/agentseek/env.py`（`apply_agentseek_env_aliases`）。完整的优先级规则见 [环境变量参考](../reference/environment.zh.md)。
