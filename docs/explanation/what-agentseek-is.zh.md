...
命令归属与合并机制见 [Choosing an entry point](choosing-an-entry-point.md)。
...
- **两个包，各做一件事。** `agentseek-cli` 让只需要脚手架与生命周期命令的团队不必
  ...
  的机制见 [Choosing an entry point](choosing-an-entry-point.md)。
- **下面是 Bub，上面是 agentseek。** agentseek 不去 fork 或替换 Bub，而是包裹它并提供
  opinionated 默认值。推理见 [How agentseek relates to Bub](bub-relationship.md)。
...
- 教程、操作指南和参考页面都假定你的项目有一个 `.agentseek/` 目录，并且 `AGENTSEEK_*` 变量
  驱动配置。为什么以及 alias 规则在 [How agentseek relates to Bub](bub-relationship.md)；
  确切的表格在 [Environment variables reference](../reference/environment.md)。
...
- 概念解释：[How agentseek relates to Bub](bub-relationship.md),
  [Choosing an entry point](choosing-an-entry-point.md)
