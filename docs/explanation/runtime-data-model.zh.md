...
> 会让 [`extension-model.md`](extension-model.md) 中的 extension 模型变得显而易见。
...
plugin。决策矩阵在 [`extension-model.md`](extension-model.md)。
...
（见 [`bub-relationship.md`](bub-relationship.md)）是一个用于解析和添加 plugin 的 uv 项目，
不是 runtime 边界。
...
- **一个底层基座，多种消费者。** 把 tape 放在中心位置意味着 debug、replay、评估、训练都从同一个
  ...
  [`../blog/introducing-agentseek.md`](../blog/introducing-agentseek.md) 中描述的赌注。
...
- 教程：[`../tutorials/03-add-a-skill-and-mcp.md`](../tutorials/03-add-a-skill-and-mcp.md)
- 操作指南：[`../how-to/add-skills.md`](../how-to/add-skills.md),
