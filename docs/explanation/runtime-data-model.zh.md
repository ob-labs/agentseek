...
> 会让 [The extension model](extension-model.md) 中的 extension 模型变得显而易见。
...
（见 [Introducing agentseek](../blog/introducing-agentseek.md)）。
...
- 默认的 Bub tape store 写到 `BUB_HOME` 内的一个本地 SQLite 文件
  ...
  见 [How agentseek relates to Bub](bub-relationship.md)）。
...
plugin。决策矩阵在 [The extension model](extension-model.md)。

### MCP —— 在配置中声明的外部 tool

**Model Context Protocol** 是 agent 调用 Python 进程外 tool 的标准方式。Bub 的 `bub-mcp`
plugin（agentseek 的硬依赖，`pyproject.toml:21`）读取一个 MCP 配置文件，并把每个声明的 server
暴露为 model 可调用的一组 tool。
...
（见 [How agentseek relates to Bub](bub-relationship.md)）是一个用于解析和添加 plugin 的 uv 项目，
不是 runtime 边界。

### Channel —— 一轮 turn 如何进出

**channel** 是接受消息进入并流式输出响应的表面。CLI（`cli`）是一种；gateway / HTTP transport
是另一种；像 Feishu、Telegram 这样的聊天平台是 plugin 提供的 channel
（`bub-feishu` 是硬依赖，`pyproject.toml:20`）。`agentseek-ag-ui` plugin 为 CopilotKit 等前端
添加一个 AG-UI SSE channel 适配器。
...
- **一个底层基座，多种消费者。** 把 tape 放在中心位置意味着 debug、replay、评估、训练都从同一个
  ...
  [Introducing agentseek](../blog/introducing-agentseek.md) 中描述的赌注。
...
- 硬依赖（Bub、`bub-feishu`、`bub-mcp`、`agentseek-schedule-sqlalchemy`）在任何 agentseek
  安装中都始终存在；其他一切都是可选的。

## 相关

- 教程：[03 — Add a skill and an MCP server](../tutorials/03-add-a-skill-and-mcp.md)
- 操作指南：[How to add skills](../how-to/add-skills.md),
...
- 概念解释：[The extension model](extension-model.md),
