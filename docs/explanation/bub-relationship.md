...
2. `apply_agentseek_cli_overrides()` — rebrand the onboard banner, swap the chat command so
   lifecycle channels are enabled, and replace the plugin-install sandbox factory.
...
The CLI behaves like Bub's, with three deliberate changes (`src/agentseek/cli.py:74-152`):
...
- `chat` is replaced so lifecycle channels (`*.lifecycle`) are enabled alongside `cli`
  ...
  CLI chat session.
- `install` resolves a fresh plugin sandbox under `.agentseek/agentseek-project` instead of
...
Everything else — `run`, `gateway`, `onboard`, `uninstall`, `update`, `login`, plus the
contrib-supplied subcommands — is Bub's own code.
...
- You can mix the two CLIs freely. `uv run bub --help` and `uv run agentseek --help` show
  the same set of commands; the difference is the banner, the chat channel set, and the
...
- If a problem reproduces under `agentseek` but not under `bub`, the suspect is one of the
  three overrides in `src/agentseek/cli.py:74-152` or the alias step in
