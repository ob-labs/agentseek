...
- `src/agentseek/env.py` —— `AGENTSEEK_*` 到 `BUB_*` 的 alias 规则，加上位置默认值
  ...
  [How agentseek relates to Bub](bub-relationship.md) 中解释。
...
关于 skill 是什么，见 [The runtime data model](runtime-data-model.md)。
...
见 [Choosing an entry point](choosing-an-entry-point.md)），其余项是 harness 的运行时 plugin。
...
（`pyproject.toml:133-139`）中都被排除。参考：[Templates reference](../reference/templates.md)。
...
- `entrypoint.sh` 和 `docker-compose.yml` 是 Docker entry point；
  见 [Choosing an entry point](choosing-an-entry-point.md)。
...
- 参考：[File layout reference](../reference/file-layout.md),
