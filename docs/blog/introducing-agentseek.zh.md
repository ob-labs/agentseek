...
agentseek 是一个 Python 发行版加一组 uv workspace 内的 contrib 包。

**核心发行版**（`src/agentseek/`）很小：
...
- `apply_agentseek_cli_overrides()` 改写 onboarding banner、让 `chat` 启用
  ...
  （`src/agentseek/cli.py:74`）。
- `__main__.py` 启动 `BubFramework` 并向它请求一个 Typer CLI
  （`src/agentseek/__main__.py:52`）。

剩下的全部都是插件。捆绑的硬依赖（`bub`、`bub-feishu`、`bub-mcp`、
`agentseek-schedule-sqlalchemy`、`logfire`）和可选 extras（`ag-ui`、`cli`、
`langchain`、`observability`、`oceanbase`、`context`）列在
[Packages 参考](../reference/packages.md) 中。整个目录布局 —— `src/`、`contrib/`、
`examples/`、`templates/`、`skills/`、`references/`、`docs/` ——
见 [文件存放位置](../explanation/where-things-live.md)。

**CLI**（`agentseek chat`、`agentseek gateway`、`agentseek install`，以及来自
`agentseek-cli` 的生命周期命令 `create / run / build / deploy / api / ctx /
skills`）是让陌生人在自己机器上看到项目跑起来的最低成本路径。它不是用来在上面盖应用
的地方；那条路径是 [first harness app 教程](../tutorials/02-first-harness-app.md)。
完整的取舍说明见 [选择一个入口点](../explanation/choosing-an-entry-point.md)。
...
有完整描述，作为运行时数据形态则在
[运行时数据模型](../explanation/runtime-data-model.md) 里说明。
...
agentseek 并 **不试图** 取代你的 agent framework。如果你已经在用 LangChain、
DeepAgents 或者自己的 orchestrator，把模型轮次走 `agentseek-langchain`，让 harness
管状态、通道和 tape 就好。如果你是新项目，捆绑的 Bub kernel 本身就够用。

正是 harness 这种形态让 [extension model](../explanation/extension-model.md) 能保持
...
`.agentseek/` 和 `AGENTSEEK_*` 是面向项目的默认值。除了 `src/agentseek/cli.py:74`
里那三处 Typer monkeypatch 之外，Bub 没有被 fork 也没有被改。完整关系见
[agentseek 和 Bub 的关系](../explanation/bub-relationship.md)。
...
- **动手体验：** [教程](../tutorials/index.md)。CLI demo 五分钟；first harness
  app 是大多数应用开发者真正想走的路径。
- **查事实：** [参考](../reference/index.md)，包括环境变量、CLI、包、文件布局、
  Docker。
- **决定改动放在哪里：** [extension model](../explanation/extension-model.md)。
- **仓库地址：** [ob-labs/agentseek on GitHub](https://github.com/ob-labs/agentseek)。
- **目录：** 站点上的 [Hub](../hub.md) 收录了插件和技能；更广的 Bub 生态请看
