...
- 在 `docker-compose.yml` 旁的 `.env` 中至少配置了
  `AGENTSEEK_MODEL` 与 `AGENTSEEK_API_KEY`。见 [How to configure the model provider](configure-model.md)。
...
- 操作指南: [How to configure the Docker workspace](configure-docker-workspace.md), [How to build and deploy](build-and-deploy.md)
