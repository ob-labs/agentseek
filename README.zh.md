...
模板是一个**持续增长的集合**——LangChain、DeepAgents 和 Bub 都在不断添加和打磨。欢迎提交 PR。
...
| `langchain/sandbox` | 带 LangSmith sandbox 后端的 DeepAgents coding agent。 |
| `deepagents/research` | DeepAgents 研究 Agent，内置 Tavily 搜索和流式报告 UI。 |
| `deepagents/content-builder` | 带品牌记忆、skills 和图片生成的 DeepAgents 内容生成 agent。 |
| `deepagents/default` | `create_deep_agent` 绑定到 `agentseek-langchain`。 |
| `bub/default` | 轻量 Bub agent + CopilotKit 前端，不含 LangChain。 |
| `bub/contextseek` | 带 ContextSeek 语义记忆层的 Bub agent。 |

运行 `agentseek create --template` 可以查看完整列表和描述。
