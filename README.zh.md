...
AgentSeek 是 database-native agent harness，对用户只暴露一个 CLI 入口：
`agentseek`。

| 目标 | 命令族 | 适合场景 |
| --- | --- | --- |
| 管理项目 | `agentseek new/dev/build/deploy` | 创建、运行、打包或部署应用。 |
...
完整命令面见 [CLI 参考](docs/reference/cli.zh.md)。
...
| 应用形状 | 从这里开始 |
| --- | --- |
| 最小 LangChain 应用 | `agentseek new langchain/markdown-messages` |
| 完整 AgentSeek 交付应用 | `agentseek new langchain/default` |
| DeepAgents research 应用 | `agentseek new deepagents/research` |
| 不带 LangChain 的 Bub 应用 | `agentseek new bub/default` |
