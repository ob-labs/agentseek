...
如果要走完整的 AgentSeek runtime 交付路径，选择更完整的模板：
...
完整目录见[模板参考](docs/reference/templates.zh.md)。LangChain、DeepAgents 和 Bub
模板都可以通过 `agentseek create --template` 浏览。
...
1. 用 `agentseek-cli` 从模板创建项目。
2. 在本地用 `langgraph dev` 或 `agentseek run` 运行生成项目。
...
| 你想做什么 | 从这里开始 |
| --- | --- |
| 构建最小 LangChain 应用 | `agentseek create langchain/markdown-messages` |
| 构建完整 AgentSeek 交付应用 | `agentseek create langchain/default` |
| 构建 DeepAgents research 应用 | `agentseek create deepagents/research` |
| 使用不带 LangChain 的 Bub 路径 | `agentseek create bub/default` |
| 理清包和入口边界 | [选择一个入口](docs/explanation/choosing-an-entry-point.zh.md) |
| 查看所有 CLI 参数 | [CLI 参考](docs/reference/cli.zh.md) |
