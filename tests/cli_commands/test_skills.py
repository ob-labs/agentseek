...
from agentseek.cli.commands import skills as skills_module
...
def test_skills_list_shows_embedded_catalogue_by_default(monkeypatch) -> None:
    ...
    result = CliRunner().invoke(build_command_app(), ["skills", "list"])
...
def test_skills_list_with_args_passes_through(monkeypatch) -> None:
    ...
    result = CliRunner().invoke(build_command_app(), ["skills", "list", "--global", "--json"])
...
def test_skills_list_agent_filter_passes_through(monkeypatch) -> None:
    ...
    result = CliRunner().invoke(build_command_app(), ["skills", "list", "--agent", "OpenClaw"])
...
def test_skills_propagates_non_zero_exit_code(monkeypatch) -> None:
    ...
    result = CliRunner().invoke(build_command_app(), ["skills", "find", "typescript"])
...
def test_skills_falls_back_to_npx(monkeypatch) -> None:
    ...
    result = CliRunner().invoke(build_command_app(), ["skills", "list", "--global"])
    assert result.exit_code == 0
...
def test_skills_reports_missing_both(monkeypatch) -> None:
    ...
    result = CliRunner().invoke(build_command_app(), ["skills", "add", "--all"])
...
def test_skills_passes_all_passthrough_subcommands(monkeypatch) -> None:
    ...
    for sub in passthrough_commands:
        result = runner.invoke(build_command_app(), ["skills", sub])
